...
int main(int argc, const char* argv[]) {
    ...
    std::vector<int> winnersBracket(100);
    ...
    for(int i = 0; i < numBalls.size(); i++) {
        for(int l = 0; l < 100; l++) {
            if(std::count(winnersBracket.begin(), winnersBracket.end(), l) < 1 && l != 0) {
                for(int j = 0; j < 5; j++) {
                    for(int k = 0; k < 5; k++){
    ...
    for(int i = 0; i < winnersBracket.size(); i++){
        std::cout << winnersBracket[i] << " ";
    }
